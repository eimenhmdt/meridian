...
### Backend Services
```bash
# Deploy Cloudflare Workers and API endpoints
...
### Brief Generation
The intelligence briefs are currently generated using a Python notebook. This process:
1. Scrapes news from configured sources
2. Processes articles through Gemini AI
3. Clusters related stories
4. Generates analytical summaries
5. Produces the final brief
...
- `apps/scrapers/.env` - Scraper configuration
- `apps/briefs/.env` - Brief generation configuration

#### Database Setup
1. Install and start PostgreSQL locally or use a cloud provider
2. Create a new database for Meridian
3. Add the database connection URL to your environment:
   ```bash
   # In packages/database/.env
   DATABASE_URL="postgres://user:password@localhost:5432/meridian"
   ```
